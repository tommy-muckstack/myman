...
Requires Node.js 22+ on the computer containing the Brain. macOS app actions require MyMan; the [Linux companion](linux-agents.md) supports X11 and Hyprland/Omarchy/Sway screenshot capture, pixel annotations and notes with owner-controlled config grants. An optional root-owned system policy caps Linux user grants; runtime code stays outside Brain. This remains consent enforcement for trusted code, not an OS sandbox. Mac-only actions return structured platform errors on Linux. The root MCP entrypoint selects the matching platform bundle. On macOS, New text-targeted markup, window recording, pause/resume and video export commands require MyMan 1.1.61; discover available actions on the installed app before use. Open MyMan to generate the local Brain export and catalog. The plugin's committed MCP bundle needs no npm install. Existing exports remain readable while MyMan is closed.
