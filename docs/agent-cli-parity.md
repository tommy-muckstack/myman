...
| One-command demo | `demo --script steps.json [--app CMD] [--dry-run]` | Mac and Linux X11, with the same steps file. The Mac opens the app by name, hides every other app while it records (`focus`, default on) and needs the control grant plus Accessibility; keys use Mac names such as `cmd+s` | Opens the app, hides other windows while it records (`focus`, default on; restored afterward), records the window, runs click/type/key/scroll steps, zooms where it acted, then polishes; Wayland is unsupported for now |
| Look before a demo | `demo --look --app NAME [--window TITLE]` | Picture of the app window, numbered copy with a 50-point grid, and buttons/fields from Accessibility plus on-screen text, each with its click point | Same output; elements come from on-screen text (tesseract); X11 only for now |
