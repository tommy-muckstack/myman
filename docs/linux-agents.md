...
- **Steps.** `wait` (seconds), `move` and `click` (`[x, y]` or the label to click, with `seconds` for the glide, `button` and `double`), `type` (text, with `cps` for characters per second and `at` for where the text appears), `key` (such as `Return` or `ctrl+s`) and `scroll` (positive scrolls down). Coordinates are relative to the top-left of the recorded area. Unknown keys are errors.
- **Click by name.** `{"click": "Play"}` finds the text "Play" in the recorded area right before the step (with `tesseract`), waiting up to 8 seconds for it to appear. The recording pauses while it reads the screen, so the video has no dead air, and pauses in the app (a page loading) are skipped too. The whole label must match, ignoring case and punctuation; `"nth": 2` picks the second match in reading order. A `type` step's `at` can be a label too. If nothing matches, the recording is cancelled and the error lists close matches. The result's `found` lists where each name was clicked. Labels that `--look` doesn't list (icons, some light-on-dark text) need points.
- **Captions.** Any step can carry a `caption`, a short line of text (up to 100 characters, at most 2 lines) shown while that step happens and until the next captioned step starts: `{"click": "Search", "caption": "Search for a song"}`. Captions are drawn in Gellix SemiBold, white on a soft rounded dark label, and fade in and out; with a backdrop they sit in a strip under the window, never over the app, and without one they sit near the bottom of the video. Captions need polish, so `"polish": false` with captions is an error. A `record polish` recipe can also take `captions: [{"text", "start", "end"}]` (seconds, up to 60).
