...
Screenshot results include the same ID, PNG/Brain paths, dimensions, scale, timestamp, timezone, attachment metadata, and job ID fields as the Mac capture subset. Tesseract OCR runs locally before the screenshot is indexed when available. `ocr_status` is `ready`, `unavailable`, or `failed`; missing OCR does not prevent capture. OCR is in the Markdown body and searchable by existing Brain tools. Image/circle/callout overlays, native UI, audio/webcam recording, meetings, dictation and the other Mac-only actions return `unsupported_on_platform`. (`capture ocr` boxes, window capture and the clipboard are covered below.)
