...
On Linux, use the [Linux agent guide](../../docs/linux-agents.md) and `myman actions` before acting. The root MCP bundle selects the Linux app server automatically. Supported writes include X11 or Hyprland/Omarchy/Sway screenshot capture; arrow/box/highlight/text/pixelate/crop annotation by explicit pixels or by on-screen text (`target_text`/`target_region` from OCR); clipboard read/write; notes and tasks; video-only screen recording of a display or region (pause/resume, frames, export and `record polish`); timers and reminders; and bundles, handoffs and leases between agents. Search uses local Brain keywords. All owner grants in `~/.config/myman/agents.json` start off; never edit them to grant yourself access. An optional root-owned `/etc/myman/agents.json` caps user grants; never modify either policy to grant yourself access. Run installed executables from `~/.local/share/myman`, never from Brain. Use the same request/job IDs to recover results. Some Linux features are CLI commands, not MCP actions: `myman meeting start` records the microphone and computer audio (needs the `recording` and `microphone` grants; pass the returned `--id` to stop it); `myman demo` records an app demo (X11 only; needs the `recording` and `control` grants); dictation comes from Voxtype after the person runs `myman dictation connect`, and saved dictations are read from the library. Mac UI, Live Text, recording with microphone, webcam or system audio or of a single window, meeting notes and settings, dictation control, scrolling capture, briefs, fonts, sharing and calendar return `unsupported_on_platform`; do not follow those Mac recipes on Linux. Target the Linux host that contains the intended Brain, never an unrelated empty cloud folder.
