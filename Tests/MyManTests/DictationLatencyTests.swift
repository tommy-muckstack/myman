...
final class DictationLatencyTests: XCTestCase {
    func testAlreadyPunctuatedNeutralSpeechSkipsGeneration() {
        ...
        XCTAssertFalse(DictationCleanup.requiresModelPolish("Revenue increased forty percent.", tone: .neutral), "Numbers are formatted by rules, not the model")
        XCTAssertTrue(DictationCleanup.requiresModelPolish("Meet at 3, actually no, make that 4.", tone: .neutral))
