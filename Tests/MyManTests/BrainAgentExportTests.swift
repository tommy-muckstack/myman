...
final class BrainAgentExportTests: XCTestCase {
    private func fixture() throws -> DatabaseQueue {
        ...
        try queue.write { db in
            ...
            try db.execute(sql: "INSERT INTO meeting(id,title,startedAt,endedAt,transcript,summary) VALUES(?,?,?,?,?,?)", arguments: ["call", "Design review", date, date.addingTimeInterval(3600), "**Jared Lane** [0:00]: Review the registration pricing.\n**Zoe Park** [0:10]: The plan costs $49.", "Discussed registration pricing."])
    ...
    func testCatalogIncludesCompleteTasksDictationThemesAndOriginalMeetingInterval() throws {
        ...
        XCTAssertTrue(markdown.contains("  - Jared Lane")); XCTAssertTrue(markdown.contains("ended: "))
        XCTAssertTrue(markdown.contains("**Zoe Park** [0:10]: The plan costs $49."))
