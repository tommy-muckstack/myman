...
When the user describes a polished app demo in plain words, make it yourself with `demo`; never ask them for coordinates. Run `demo --look --app NAME --json` first. It returns a picture of the window, a numbered copy with a 50-point grid, and `elements` with each label's `click` point in window points. Match the description to the numbered elements (read icons off the grid), write the steps file (prefer `{"click": "Label"}` with the exact label from `elements`; use `[x, y]` only for unlabelled icons), check it with `demo --script steps.json --dry-run --json`, run it, and review the result with `record frames`. Fix and rerun any step that missed. If the recording, control or Accessibility grant is off, tell the user which switch to turn on. Give each step a short `caption` saying what is happening in plain words ("Search for a song"); it shows under the window in Gellix while that step runs, so viewers follow without sound.
