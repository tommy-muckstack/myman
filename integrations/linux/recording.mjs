...
export async function start(args) {
  ...
  if (desktop.session === 'wayland') {
    ...
    // duration flag, so GNU timeout enforces max_duration with the same SIGINT
    // finalization, and forwards our SIGINT on stop.
    const recorder = [deps['wf-recorder'], ...(await wfRecorderNoDamage(deps['wf-recorder']) ? ['-D'] : []), '-y', '-g', `${x + (desktop.origin?.[0] ?? 0)},${y + (desktop.origin?.[1] ?? 0)} ${w}x${h}`, '-c', 'libx264', '-p', 'preset=veryfast', '-x', 'yuv420p', '-f', file];
    const timeout = await command('timeout');
    [file_, argv] = timeout ? [timeout, ['--signal=INT', '--kill-after=10', String(max), ...recorder]] : [recorder[0], recorder.slice(1)];
  } else {
  ...
  child.unref();
  const session = { session_id, state: 'recording', backend: desktop.session === 'wayland' ? 'wf-recorder' : 'ffmpeg', started_at: new Date().toISOString(), max_duration: max, region: [x, y, w, h], width: w, height: h, pid: child.pid, process_identity: await processIdentity(child.pid), file };
  await save(session);
...
export async function stop({ session_id }) {
  ...
  const probed = info && info.size >= 1024 ? await probe(session.file) : null;
  // Never leave a session stuck in "recording" or discard a playable file.
  if (!info || info.size < 1024 || (forced && probed === null)) {
    session.state = 'failed'; await save(session); await rm(session.file, { force: true });
    fail(forced ? 'PROCESSING_TIMEOUT' : 'BACKEND_FAILED', forced ? 'The recorder did not finalize within 15 seconds and its video was unreadable. Start a new recording.' : 'The recorder produced no usable video.');
  }
  const duration = probed ?? elapsed(session);
  const saved = await saveRecording({ file: session.file, width: session.width, height: session.height, duration, started_at: session.started_at, backend: session.backend });
  const result = forced ? { ...saved, warnings: ['The recorder was force-stopped after 15 seconds; the saved video probed as readable but may end early.'] } : saved;
