...
enum QuickSpokenMath {
    ...
    private static func number(_ words: [String]) -> String? {
    ...
    static func numberValue(_ text: String) -> Double? {
        if let value = Double(text), value.isFinite { return value }
        if text == "a" || text == "an" { return 1 }
        let words = text.lowercased().replacingOccurrences(of: "-", with: " ").split(whereSeparator: \.isWhitespace).map(String.init)
        return number(words).flatMap(Double.init)
    }
