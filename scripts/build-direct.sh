...
VERSION="${RELEASE_VERSION:-1.1.24}"
BUILD_NUMBER="${RELEASE_BUILD:-36}"
...
APPCAST
    ...
    if [[ -n "${BLOB_READ_WRITE_TOKEN:-}" ]]; then
        ...
        vercel blob put "$DMG_PATH" --pathname "$VERSIONED_DMG_NAME" --access public \
            --allow-overwrite true --content-type application/x-apple-diskimage \
            --cache-control-max-age 31536000
        vercel blob put "$DMG_PATH" --pathname "$SLUG.dmg" --access public \
            --allow-overwrite true --content-type application/x-apple-diskimage \
            --cache-control-max-age 300
        vercel blob put "$APPCAST_PATH" --pathname "$SLUG-appcast.xml" --access public \
            --allow-overwrite true --content-type application/rss+xml \
            --cache-control-max-age 300
    else
